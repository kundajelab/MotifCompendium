...
def plot64_from_motifs_parallel(motifs, parallel):
...
def generate_report(data_list, output_file, parallel=True):
	# Use Agg backend
	current_backend = matplotlib.get_backend()
	matplotlib.use('Agg')

	# Make plots
	plot_dict = {x: plot64_from_motifs_parallel(m, parallel) for x, m in enumerate(data_list)}
	...
	rendered_html = template.render(data=plot_dict, enumerate=enumerate)
