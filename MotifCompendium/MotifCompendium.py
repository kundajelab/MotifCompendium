...
class MotifCompendium:
    ...
    def update_from_summary_table(self, html_loc: str) -> None:
        ...
        Parses the rc chunk data embedded in the HTML file and updates
        self.metadata with any changes found in non-image columns.
        ...
        Raises:
            ...
            ValueError: If no rc chunks or _META payload can be found (e.g. file
                from an older version of the tool; regenerate via summary_table_html).
        """
        import re, json
        ...
        image_column = meta['image_column']
    ...
    def assign_label_from_motifs(
        ...
        Args:
            ...
            utf8_images: A dict of utf8 images for each motif in reference_motifs, with keys
              "logo (fwd)" and "logo (rev)" for the forward and reverse complement logos.
              If saved_images is True and utf8_images is None, the logos will be generated
              on the fly using the trimming option logo_trimming. If saved_images is
              True and utf8_images is a dict of utf8 images, these images will be used
        ...
        if (utf8_images is not None) and not (
            (isinstance(utf8_images, dict))
            and all(key in utf8_images for key in ["logo (fwd)", "logo (rev)"])
            and len(utf8_images["logo (fwd)"]) == len(reference_motifs)
            and len(utf8_images["logo (rev)"]) == len(reference_motifs)
        ):
            raise ValueError(
                "utf8_images must be a dict with keys 'logo (fwd)' and 'logo (rev)' that contain lists of the same length as reference_motifs."
            )
        # Store as copys
        ...
        for i in range(max_submotifs):
            ...
            if save_images:
                ...
                if utf8_images is not None and not save_all_matches:
                    ...
                    match_image = []
                    for n, idx in enumerate(match_idxs[i]):
                        if idx < 0:
                            match_image.append(None)
                        else:
                            image_key = "logo (rev)" if match_alignment_rcs[i][n] else "logo (fwd)"
                            match_image.append(utf8_images[image_key][idx])
                    self.__images[f"{save_col_prefix}_logo{i}"] = match_image
                else:
