...
def setup_parser():
    ...
    parser.add_argument('--sim-scan', type=list, default=None, help='List of similarity thresholds to scan during clustering. MUST INCLUDE SIM_THRESHOLD.')
...
def apply_filter_threshold(mc: MotifCompendium, flag_col:str, 
    metric: str, operation: str, threshold: float, override: bool) -> None:
    if override:
        if operation == '<':
            mc.metadata[flag_col] = mc.metadata[flag_col] & (mc.metadata[metric] < threshold)
        elif operation == '<=':
            mc.metadata[flag_col] = mc.metadata[flag_col] & (mc.metadata[metric] <= threshold)
        elif operation == '>':
            mc.metadata[flag_col] = mc.metadata[flag_col] & (mc.metadata[metric] > threshold)
...
if __name__ == '__main__':
    ...
    # Set compute options
    if args.verbose:
        print(f'Setting compute options:\n'
    ...
    while True:
        try:
            ...
            if args.verbose:
                print(f'Completed building MotifCompendium object:\n'
    ...
    if args.metadata:
        ...
        if len(metadata_df) == len(mc.metadata):
            ...
            mc.metadata = mc.metadata.merge(metadata_df, left_index=True, right_index=True, how='left')
        elif len(metadata_df) == len(set(args.input_names)):
            mc.metadata = mc.metadata.merge(metadata_df, left_on="model", right_on=metadata_df.columns[0], how='left')
        else:
            raise ValueError('Number of rows in metadata file must either match \
    ...
    if args.reference.endswith('.mc'):
        ...
        raise ValueError('Reference file must be a MotifCompendium object or PFM, MEME .txt file.')
    ...
    if args.verbose:
        print(f'Overriding flags for matches above threshold:\n'
    ...
    if args.verbose:
        print(f'Saving filtered MotifCompendium objects:\n'
              f'  Filtered: {mc_path}\n'
    ...
    for sim_threshold in sim_thresholds:
        ...
        for iter, mc_iter in mc_dict.items():    
            ...
            if args.time:
                print(f'Time taken: {time.time() - start_time:.2f}s')
            if args.verbose:
                print(f'Total number of clusters ({cluster_col_name}): {len(mc_iter.metadata[cluster_col_name].unique())}')
        ...
        if quality:
            ...
            if args.time:
                start_time = time.time()
            sns.heatmap(mc.clustering_quality(cluster_col=cluster_col_name,), cbar=True)
