...
def _compute_similarity_jss(
    ...
    js_similarity = (np.sum(motifsP_flat * np.where(motifsP_flat == 0, 0, np.log2(motifsP_flat)), axis=1)[:, np.newaxis] \
        + np.sum(motifsQ_flat * np.where(motifsQ_flat == 0, 0, np.log2(motifsQ_flat)), axis=1)[np.newaxis, :])[:, np.newaxis, :] # (N, 1, M)
    ...
    mixed_tensorM_log = np.where(mixed_tensorM_log == 0, 0, np.log2(mixed_tensorM_log)) # (N, K, 59, 88, M)
