...
def load_modiscos(
    ...
    if num_processes == 1 or len(modisco_dict) == 1:
        ...
        for m_name, m_loc in modisco_dict.items():
            ...
            m_locs.append(m_loc)
        payloads = [
            (m_loc, score_type, load_subpatterns, modisco_region_width) for m_loc in m_locs
        ]
...
def load_modisco(
    ...
    Each motif from a Modisco results file is extracted/
      The motif names, number of seqlets per motif, whether the motifs are positive or
      negative patterns, the mean seqlet distance with respect to the Modisco region
      start position, and the mean seqlet contribution are also returned.

    Args:
        modisco_file: A Modisco file path.
        score_type: The type of scores to use for motif extraction. 
          Option:"contrib_scores", "hypothetical_contribs", or "sequence".
        load_subpatterns: Whether or not to load subpatterns from the Modisco file. If
          True, motifs will be loaded at the subpattern level. If False, motifs will be
          loaded at the pattern level.
        modisco_region_width: The region width used during Modisco. This argument only
    ...
    Notes:
        ...
        Motifs are returned as a normalized (N, 30, 4) 4 channel motif stack.
    """
    motifs, motif_names, seqlet_counts, posnegs, avgdist_summits = (
        [],
        [],
        [],
        [],
        ...
        for pattern_type in ["pos_patterns", "neg_patterns"]:
            ...
            for pattern in list(f[pattern_type]):
                ...
                # Main patterns
                    ...
                    seqlets = f[pattern_type][pattern]["seqlets"][score_type][()]
                    # motif_sim = _sequence_importance_from_seqlets(seqlets, ic)
                    motif_sim = np.mean(seqlets, axis=0)
                    motifs.append(motif_sim)
    ...
    return motifs, motif_names, seqlet_counts, posnegs, avgdist_summits
...
def load_pfms(
    ...
    if num_processes == 1 or len(pfm_dict) == 1:
        ...
        for p_name, p_loc in pfm_dict.items():
            p_motifs, p_motif_names = load_pfm(p_loc, motif_length=motif_length)
...
def load_pfm(
    ...
    Each Position Frequency Matrix (PFM) from the PFM file is extracted.
      The motif names are also returned. Files in PFM or MEME format are supported.
            ...
            if active_pfm:
                if x.startswith(">"):
                    ...
                    if motif_length is not None:
                        current_pfm_np = utils_motif.resize_motif(
                            current_pfm_np, motif_length
                        )
                    longest_motif_length = max(
                        longest_motif_length, current_pfm_np.shape[0]
                    )
                    pfms.append(current_pfm_np)
                    ...
                    current_pfm["A"].append(acgt[0, 0])
                    current_pfm["C"].append(acgt[0, 1])
                    current_pfm["G"].append(acgt[0, 2])
                    current_pfm["T"].append(acgt[0, 3])
            else:
                assert x.startswith(">")
                active_pfm = True
                current_pfm_name = x[1:]
                current_pfm = {"A": [], "C": [], "G": [], "T": []}
    # Resize
    pfms = [utils_motif.pad_motif(x, longest_motif_length) for x in pfms]
    pfms = np.stack(pfms, axis=0)
    return pfms, names
