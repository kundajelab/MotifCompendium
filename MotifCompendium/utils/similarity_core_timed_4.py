...
def _compute_similarity(motif_set_1, motif_set_2):
    ...
    out = np.zeros((2*L-1, N, M))
    for i in range(L):
        # q = motif_set_1[:, i].copy()
        for j in numba.prange(L):
            # z = motif_set_2[:, j].copy()
            # tempout = np.zeros((N, M))
            # for m in range(M):
            #     for k in range(K):
            #         tempout[:, m] += q[:, k] * z[m, k]
            # out[i+j] += tempout
            out[i+j] += motif_set_1[:, i].copy()@motif_set_2[:, j].copy().T
    out = np.transpose(out, (1, 2, 0))
    best_similarity, best_alignments = _max_and_argmax_along_axis1(out)
    # best_similarity = np.max(out, axis=2)
    # best_alignments = np.argmax(out, axis=2) - (L - 1)
...
print("NUMBA")
_compute_similarity(np.random.rand(10, 30, 8), np.random.rand(10, 30, 8))
print(_compute_similarity.inspect_llvm(_compute_similarity.signatures[0]))
assert(False)